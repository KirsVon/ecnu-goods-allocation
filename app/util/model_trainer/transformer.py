...
from tensorflow.compat.v1.nn.rnn_cell import DropoutWrapper, GRUCell
...
# 引入内部库
from app.util.model_trainer.sampler_transformer import *


class TransformerGRU:
    def __init__(self,
                 q_set=None,  # query集,二维数组
                 t_set=None,  # answer集,二维数组
                 dict_set=None,  # 字典集，[词：index]
                 vec_set=None,  # 向量集，[向量]，与dict_set顺序一致
                 batch_size=None,  # 训练批次
                 hidden_num=64,  # 隐藏层个数
                 attention_num=128,  # 注意力机制的数目
                 learning_rate=0.0003,  # 学习率
                 epoch_steps=100,  # 训练迭代次数
                 gamma=20,  # 余弦相似度平滑因子
                 is_train=True,  # 是否进行训练
                 is_extract=False,  # 是否进行t特征提取任务
                 is_sample=True  # 是否采用随机采样方法进行训练
                 ):
        # 外部参数
        self.q_set = q_set
        self.t_set = t_set
        ...
        self.keep_prob = 0.85
        self.params = {'num_layers': 5, 'num_heads': 8, 'keep_prob': self.keep_prob, 'hidden_size': hidden_num * 2}
        ...
        self.q_max_length = 0
        self.t_max_length = 0
        self.model_save_name = os.path.abspath('.') + "/app/util/model_trainer/dssmt"
        self.model_save_checkpoint = os.path.abspath('.') + "/app/util/model_trainer/checkpoint"
